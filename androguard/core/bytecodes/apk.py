...
class APK:
    ...
    """
    def __init__(self, filename, theKeytoolPath, decompressDir, theLogger=Logger(),raw=False, mode="r", magic_file=None, zipmodule=ZIPMODULE):
        self.filename = filename
        self.main_dir = None
        self.file_shortname = os.path.basename(filename)
        self.log = theLogger
	    ...
	    # added by Hu Wenjun <mindmac.hu@gmail.com>
        self.md5 = None
        self.keytool_path = theKeytoolPath
        self.decompress_path = decompressDir
        self.cert = {}

        self.zip = None
        self.zipFailed = False
	...
	self.check_time = None
        ...
        for i in self.zip.namelist():
            ...
            # META-INF/CERT.RSA
            if os.path.splitext(i)[-1] == '.RSA':
                self.__create_cert(i)

    def __create_cert(self, zipfile):
        if not os.path.exists(self.decompress_path):
            os.makedirs(self.decompress_path)
        cert_file = os.path.join(self.decompress_path, zipfile.split('/')[-1])
        try:
            file_obj = open(cert_file,'wb')
        except IOError:
            ex = traceback.format_exc()
            self.log.exce(ex) 
        try:
            file_obj.write(self.zip.read(zipfile))
        except IOError:
            ex = traceback.format_exc()
            self.log.exce(ex) 
        finally:
        ...
        try:
            ...
            args = '%s -printcert -file %s' % (self.keytool_path, cert_file)
            keytool_process = subprocess.Popen(args,shell=True,
                            stdout=subprocess.PIPE,
                            stdin=subprocess.PIPE,
                            stderr=subprocess.PIPE)
            std_out = keytool_process.stdout.readlines()
            std_err = keytool_process.stderr.readlines()
            if std_err and not std_out:
                self.log.exce(std_err)
            else:
                # basic info, CN=QQ, OU=QQ... 
                line = std_out[0]
                line = line.split(':')[-1]
                elements = line.split(',')
                for element in elements:
                    element = element.strip()
                    self.cert[element.split('=')[0]] = element.split('=')[-1]
    ...
    def get_file_shortname(self):
        """
            Return the short name of the APK file
        """
        return self.file_shortname
...
# added by Hu Wenjun <mindmac.hu@gmail.com>
    def get_file_size(self):
        ...
        try:
            ...
            if file_size < 1024:
                file_size = '%.2f %s' % (file_size, 'B')
            elif file_size < 1024 * 1024:
                file_size = file_size/1024
                file_size = '%.2f %s' % (file_size, 'Kb')
            else:
                file_size = file_size/(1024*1024)
                file_size = '%.2f %s' % (file_size, 'Mb')
    ...
    def get_appname_icon(self):
        """
        Get application name and icon path
        """
        try:
            app_name = ''
            icon_path = ''
            aapt_exe = os.path.join(self.main_dir, 'resources/tools/aapt')
            args = [aapt_exe,'d','badging',self.filename]
            aapt_process = subprocess.Popen(args,
                                    stdout=subprocess.PIPE,
                                    stdin=subprocess.PIPE,
                                    stderr=subprocess.PIPE)
            std_out = aapt_process.stdout.readlines()
            std_err = aapt_process.stdout.read()
    ...
    def get_exposed_components(self, tag_name):
        """
        find exposed components
        """
        exposed_components = []
        protect_permissions = []
        if not self.xml.has_key('AndroidManifest.xml'):
            return exposed_components
        
        permission_items = self.xml['AndroidManifest.xml'].getElementsByTagName('permission')
        for permission_item in permission_items:
            protect_permission = permission_item.getAttribute('android:protectionLevel')
            if (protect_permission == 'signature' or protect_permission == 'signatureOrSystem'):
                protect_permissions.append(protect_permission)
                       
        components = self.xml['AndroidManifest.xml'].getElementsByTagName(tag_name)
        for component in components:
            is_exposed = False
            component_name = component.getAttribute('android:name')
            if component.getAttribute('android:exported'):
                continue
                
            if component.getAttribute('android:permission') in protect_permissions:
                continue
                
            # need changes     
            children =  component.childNodes
            for child in children:
                if is_exposed:
                    break
                if child.nodeName == 'intent-filter':
                    for intent_child in child.childNodes:
                        if intent_child.nodeType == 1:
                            is_exposed = True
                            if component_name.startswith('.'):
                                component_name = ('%s%s' % (self.package,component_name)) 
        ...
        if tag_name == 'activity':
            ...
            if mainActivity:
                if mainActivity.startswith('.'):
                    mainActivity = '%s%s' % (self.package, mainActivity)
                if mainActivity in exposed_components:
                    exposed_components.remove(mainActivity)
                                           
        return exposed_components
    
    def get_component_action(self, tag_name):
        """
        Return component's action used in <intent-filter> by tag_name
        """
        actions = []
        
        items = self.xml['AndroidManifest.xml'].getElementsByTagName(tag_name)
        for item in items:
            action_items = item.getElementsByTagName('action')
            for action in action_items:
                actions.append(action.getAttribute('android:name'))
    ...
    def getBasicInfo(self):
        """
        Get basic information of the apk file
        """
        apkInfo = {}
        #apkInfo['FileName'] = self.file_shortname
        apkInfo['MD5'] = self.md5.upper()
        apkInfo['Package'] = self.package
        apkInfo['MinSDKVersion'] = self.get_min_sdk_version()
        apkInfo['TargetSDKVersion'] = self.get_target_sdk_version()
        
        return apkInfo
    
    def get_intent_filter(self, tag_name):
        """
        Get component's intent filter, component's type indictated by tag_name
        """
        intent_filters = {} # key: component name, value: list of intent filters
        
        if not self.xml.has_key('AndroidManifest.xml'):
            self.log.info('No AndroidManifest Found!', setTime=True)
        ...
        for component in components:
            component_name = component.getAttribute('android:name')
            if component_name.startswith('.'):
                component_name = '%s%s' % (package_name, component_name)
            
            if not intent_filters.has_key(component_name):
                intent_filters[component_name] = []
                
            for component_child in component.childNodes:
                if component_child.nodeName == 'intent-filter':
                    for intent_child in component_child.childNodes:
                        if intent_child.nodeType != Node.TEXT_NODE:
                            intent_filters[component_name].append((intent_child.nodeName,
                                                                   intent_child.getAttribute('android:name'))) 
                    break
