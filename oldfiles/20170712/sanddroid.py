...
class AppGettingThread(Thread):
    ...
    def run(self):
        try:
            while True:
		'''mac = str(uuid.UUID(int = uuid.getnode()).hex[-12:].upper())
		#self.sandDroid.log.error("macAddress:"+str(mac))
		#self.sandDroid.log.error("MACAddress:"+MAC)
		if mac != MAC:
			self.sandDroid.log.error("mac address error, please contact system administrattor")
			break
		currenttime = time.time()
		if currenttime - int(TIME) > 7776000:
			self.sannDroid.log.error("time is over, please contact system administrator")
			break'''
...
class SandDroid():
    def __init__(self, theConfigFilePath, theLogger=Logger()):
        ...
        self.numThreads = 1
        self.maxThreadRuntime = 280 #have changed 300 from 600
    ...
    def __isConfigValid(self):
        ...
        if (not self.configParser.getDbUsr()) and (not self.configParser.getDbHost()) and (not self.configParser.getDbPort()) \
    ...
    def __isDbExist(self, session, fileMd5, app):  
        apkDb = session.query(APK).filter(and_(func.upper(APK.file_md5) == fileMd5,
                                               APK.analyzed == True)).first()
        # if analyzed, remove
        if apkDb:
            try:
                os.remove(app)
    ...
    def __buildRunnerThread(self, threadIndex, apkObj, decompressDir, threadLogger):
        avdName = '%s-%s' % ('SandDroid', threadIndex+1)
        ...
        for i in xrange(self.numThreads):
            ...
            # Check how long thread is running
                ...
                if runningTime.seconds > self.maxThreadRuntime:
                    self.log.info('Thread %d for %s is running more than %d sec, cancel' % ((i+1), self.threadList[i].apkObj.get_filename(), self.maxThreadRuntime))
            ...
            while True:
                try:
                    ...
                    if self.numRunningThreads < self.numThreads and len(self.appList): 
                        ...
                        self.log.info('Free thread found (%d) for analyzing %s' % (threadIndex+1, os.path.basename(app)))
