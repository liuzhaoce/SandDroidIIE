...
class PermissionToSimulate:
    """
    Class with constants for which permissions should be simulated
    """
    LOCATION = ['ACCESS_COARSE_LOCATION','ACCESS_FINE_LOCATION']
    BATTERY = ['BATTERY_STATS','DEVICE_POWER']
    CALL_PHONE = ['CALL_PHONE','READ_PHONE_STATE']
    SMS = ['RECEIVE_SMS']
...
class EmulatorTelnetClient:
    ...
    def __runCommand(self, theCmd):
        ...
        self.log.info('Command to sent: %s\n' % theCmd, setTime=True)
        self.tn.write('%s\n' % theCmd)
        self.tn.write('exit\n')
        aTnOutStr = self.tn.read_all()
        self.tn.close()
        self.log.info('Command out:\n%s' % aTnOutStr, setTime=True)
